...
describe('listProjectSessions', () => {
  it('selects the newest canonical generation and preserves fork metadata', () => {
    ...
    writeFileSync(join(dir, 'session.v3.jsonl.zstd'), zstdCompressSync(readFileSync(join(dir, 'session.jsonl'))))
    writeSessionLog(dir, 'child', 1, 'stale')
    writeFileSync(join(dir, 'session.v03.jsonl'), 'ignored')
    writeFileSync(join(dir, 'session.v4.jsonl.tmp'), 'ignored')
    expect(listProjectSessions(home, '/proj')[0]).toMatchObject({ title: 'current', parentSession: 'parent', cwd: '/proj' })
    writeFileSync(join(dir, 'session.v3.jsonl.zstd'), 'corrupt')
    expect(listProjectSessions(home, '/proj')[0]?.title).not.toBe('stale')
    writeFileSync(join(dir, 'session.v4.jsonl'), readFileSync(join(dir, 'session.jsonl')))
    expect(listProjectSessions(home, '/proj')[0]?.unreadable).toBe(true)
    expect(newlyCreatedEmptySessions(new Set(), listProjectSessions(home, '/proj'))).toEqual([])
  })
