...
export function reduceSessionEvent(state: ReducerState, event: SessionEvent): ReducerState {
  ...
  switch (event.type) {
    ...
    case 'tool/code-dispatch': {
      ...
      // Official 0.1.2-alpha.3 dispatches do not expose presentation meta. Keep
    ...
    case 'request/header': {
      // The effective reasoning effort resolved for the last request (adapter
      // default already materialized); last-wins so a switch to a non-reasoning
      // model clears it.
      const effort = event.data.header.config.reasoningEffort
      return { ...base, reasoningEffort: effort === undefined ? undefined : String(effort) }
    }

    case 'request/context':
      return { ...base, contextWindow: event.data.contextWindow }
